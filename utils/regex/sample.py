...
def get_sample_hdfc_regex_token_sequence(debug=False):
    ...
    token_sequence.push_token(
        NamedToken(RegexToken(token=Token.PHRASE, max_len=1, multiline=True, alignment=Alignment.LEFT), "Description"))
    token_sequence.push_token(RegexToken(token=Token.WHITESPACE_HORIZONTAL, min_len=10, max_len=90))
    token_sequence.push_token(NamedToken(RegexToken(token=Token.WORD, min_len=15, max_len=16), "ReferenceNum"))
    token_sequence.push_token(RegexToken(token=Token.WHITESPACE_HORIZONTAL, max_len=1))
    ...
    token_sequence.push_token(NamedToken(RegexToken(token=Token.NUMBER, min_len=1, max_len=20), "Balance"))
